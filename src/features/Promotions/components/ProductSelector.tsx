...
export const ProductSelector: React.FC<ProductSelectorProps> = ({ products, onSelectedProductsChange }) => {
  const [applicationType, setApplicationType] = useState<'all' | 'individual' | 'category' | 'expiration'>('all');
  const [selectedCategories, setSelectedCategories] = useState<string[]>([]);
  const [selectedConditions, setSelectedConditions] = useState<string[]>([]);
  const [selectedDeliveryMethods, setSelectedDeliveryMethods] = useState<string[]>([]);
  const [selectedIndividualProducts, setSelectedIndividualProducts] = useState<number[]>([]);
  ...
  const deliveryMethods = Array.from(new Set(products.map(p => p.deliveryMethod)));
  ...
  const handleCheckboxChange = (category: 'categories' | 'conditions' | 'deliveryMethods', item: string) => {
            ...
            <div>
              ...
              {categories.map(category => (
                <div key={category} className="flex items-center space-x-2 mb-2">
                  ...
                  />
                  <label htmlFor={category}>{category.replace(/_/g, ' ').split(' ').map(word => word.charAt(0).toUpperCase() + word.slice(1).toLowerCase()).join(' ')}</label>
                </div>
              ))}
            </div>
            <div>
              ...
              {conditions.map(condition => (
                <div key={condition} className="flex items-center space-x-2 mb-2">
                  <Checkbox 
                    ...
                    className="border-gray-300"
                  />
                  <label htmlFor={condition}>{condition.replace(/_/g, ' ').split(' ').map(word => word.charAt(0).toUpperCase() + word.slice(1).toLowerCase()).join(' ')}</label>
                </div>
              ))}
            </div>
            <div>
              ...
              {deliveryMethods.map(method => (
                <div key={method} className="flex items-center space-x-2 mb-2">
                  <Checkbox 
                    ...
                    onCheckedChange={() => handleCheckboxChange('deliveryMethods', method)}
                    className="border-gray-300"
                  />
                  <label htmlFor={method}>{method.replace(/_/g, ' ').split(' ').map(word => word.charAt(0).toUpperCase() + word.slice(1).toLowerCase()).join(' ')}</label>
                </div>
              ))}
            </div>
          </div>
      ...
      case 'expiration':
        return (
          <div className="mt-4">
            ...
            <RadioGroup value={expirationCriteria.type} onValueChange={(value: 'specific' | 'range' | 'relative') => handleExpirationUpdate({type: value})}>
              <div className="flex items-center space-x-2">
                <RadioGroupItem value="specific" id="specific" className="border-gray-300 text-primary-foreground focus:ring-primary"/>
                <Label htmlFor="specific">Specific Date</Label>
              </div>
              <div className="flex items-center space-x-2">
                <RadioGroupItem value="range" id="range" className="border-gray-300 text-primary-foreground focus:ring-primary"/>
                <Label htmlFor="range">Date Range</Label>
              </div>
              <div className="flex items-center space-x-2">
                <RadioGroupItem value="relative" id="relative" className="border-gray-300 text-primary-foreground focus:ring-primary"/>
            ...
            {expirationCriteria.type === 'specific' && (
              <Input 
                ...
                onChange={(e) => handleExpirationUpdate({specificDate: e.target.value})}
                className="mt-2"
              />
            )}
            {expirationCriteria.type === 'range' && (
              <div className="flex space-x-2 mt-2">
                <Input 
                  ...
                  onChange={(e) => handleExpirationUpdate({dateRange: {...expirationCriteria.dateRange, start: e.target.value}})}
                />
                <Input 
                  type="date"
                  value={expirationCriteria.dateRange.end}
                  onChange={(e) => handleExpirationUpdate({dateRange: {...expirationCriteria.dateRange, end: e.target.value}})}
                />
              </div>
            )}
            {expirationCriteria.type === 'relative' && (
              <div className="flex space-x-2 mt-2">
                <Input 
                  ...
                  onChange={(e) => handleExpirationUpdate({relativePeriod: {...expirationCriteria.relativePeriod, value: e.target.value}})}
                  placeholder="Number"
                />
                <Select 
                  value={expirationCriteria.relativePeriod.unit}
                  onValueChange={(value: 'days' | 'weeks' | 'months') => handleExpirationUpdate({relativePeriod: {...expirationCriteria.relativePeriod, unit: value}})}
                >
                  <SelectTrigger>
