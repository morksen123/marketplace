          ...
          <div className="flex-grow overflow-y-auto p-4">
            {selectedChat.messages?.reduce((acc: JSX.Element[], msg, index, array) => {
              ...
              acc.push(
                ...
                >
                  <div className={`max-w-[70%] ${isDistributorMessage ? 'ml-auto' : 'mr-auto'}`}>
                    {msg.text && (
                      <div className="flex flex-col">
                        ...
                        <span className={`text-xs mt-1 ${isDistributorMessage ? 'text-right' : 'text-left'} text-gray-500`}>
                        ...
                        {msg.images.map((image, index) => (
                          image.match(/\.(jpeg|jpg|gif|png)$/i) ? (
                            <div key={index} className="mt-2">
                              ...
                              />
                              <span className={`text-xs mt-1 ${isDistributorMessage ? 'text-right' : 'text-left'} text-gray-500 block`}>
                                {format(messageDate, 'h:mm a')}
                              </span>
                            </div>
                          ) : (
                            <div key={index} className="mt-2">
                              <div 
                                className={`flex items-center rounded p-2 cursor-pointer ${isDistributorMessage ? 'bg-green-500 text-white': 'bg-gray-200'}`}
                                onClick={() => handleFileClick(image)}
                              >
                                {getFileIcon(image)}
