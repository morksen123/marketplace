...
export const BuyerRefundsList: React.FC<BuyerRefundsListProps> = ({ refunds: buyerRefunds }) => {
  ...
  const filteredRefundsMemo = useMemo(() => {
    return buyerRefunds?.filter(refund => {
  ...
  }, [buyerRefunds, searchTerm, activeFilter, sortOrder]);
  ...
  }, [buyerRefunds, searchTerm, activeFilter, filteredRefundsMemo]);
          ...
          {filteredRefunds.slice(indexOfFirstRefund, indexOfLastRefund).map((refund) => (
            <div key={refund.refundId} className="flex items-center justify-between p-4 bg-white border border-gray-200 rounded-md shadow-sm">
              ...
              <div className="flex space-x-2 ml-4">
                <Link to={`/buyer/orders/refunds/${refund.refundId}`}>
